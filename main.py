...
class Board:
    ...
    def add_ship(self, row, col, route, length):
        try:
            @staticmethod
            def check_used_ships(limit, length):  # Если корабль доступен в списке, возвращает список, вычеркивая из него этот корабль
                flag = False
                for i in limit:
                    if len(i) == length:
                        flag = True
...
class Game:
        ...
        if self.whose_move:
            ...
            while True:
                try:
                    ...
                    if is_hit:
                        if hp == 0:
                            print('Корабль врага убит!\n')
                        else:
                            print('Корабль врага ранен!\n')
                        sleep(2)
                        self.ai.moves.get_enemy_board()
                        print('-------------')
                        self.user.moves.get_my_board()
    ...
    def greet(self):
        ...
        print('Вначале игры у вас будет выбор, самому распределить корабли на доске или случайным образом.')
